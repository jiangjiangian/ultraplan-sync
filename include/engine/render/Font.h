...
inline const char* UiLiteralChars() {
    return
        ...
        // 表意字若同時不在 docs/content/*.md 與此字面集中，便會落到 raylib 無字符的
        ...
        // 出現在 docs/content/*.md，但 訪（拜訪）不在，缺它說明面板便會缺字成 `?`
        ...
        // docs/content/*.md 中，故只在此進圖集。對應的字符掃描測試會列舉這些選單
        // 標籤，遇任何未涵蓋字即讓建置失敗，故此處為驗證而非臆測——機制同前。
        "回束"
        // 加強版結局「畫面」（EndingView.cpp）與「遊戲說明」中可操作的結局段落
        // （GameHelp.h）會繪製少數不在任何 docs/content/*.md 中的字，故只在此進圖集。
        ...
        // 字串。下列少數字出現在這些程式碼字面值中，卻不在任何 docs/content/*.md 中，
        // 故只在此進圖集；掃描會加以驗證。
        //   北 目標「地圖東北」   垂 說明「傘面下垂」   紫 說明「暗紫色」
        //   幸 圖鑑「小確幸」     血 圖鑑「三頁心血」
        "北垂紫幸血"
        // 結局 D 卡片（EndingView）、ItemCatalog 效果說明（CatalogStrings）與第四章
        // 結局「自白」對話新用到、且不在任何 docs/content/*.md 中的字，故只在此進
        ...
        // "…" 字串字面值中的每個 CJK 碼位、以及每個 docs/content/*.md，除非每個碼位皆
        ...
        // 任何 docs/content/*.md 中，故只在此進圖集（否則全字面值掃描會讓建置失敗——
 ...
 *
 * 必含 ASCII 32..126，再加上每個 docs/content/*.md 檔的每個碼位，以及寫死的 UI
