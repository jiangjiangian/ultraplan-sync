...
// Length 回傳歐幾里得長度。
TEST_CASE("Vec2::Length 回傳歐幾里得長度") {
    Vec2 v{3.0f, 4.0f};
    CHECK(v.Length() == doctest::Approx(5.0f));
}

// Normalized 回傳單位向量；零向量則回傳零向量（避免除以零）。
TEST_CASE("Vec2::Normalized 回傳單位向量；零向量則回傳零向量") {
    Vec2 v{0.0f, 0.0f};
    auto n = v.Normalized();
