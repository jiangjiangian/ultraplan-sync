...
TEST_CASE("HotPack Consume：karma +5、雨量 -25、isActive false、發出 ShowMessage") {
    Player p({0, 0});
...
TEST_CASE("WaterproofSpray Consume：雨量 -35、不影響 karma、isActive false") {
    Player p({0, 0});
...
TEST_CASE("EnergyDrink Consume：karma +3、雨量 -15、isActive false") {
    Player p({0, 0});
...
TEST_CASE("Consume 對 null 玩家是空操作（不崩潰，道具維持啟用）") {
    MessageCapture cap;
    cap.Attach();
...
TEST_CASE("Factory 產生具正確動態型別的 HotPack") {
...
TEST_CASE("Factory 產生具正確動態型別的 WaterproofSpray") {
...
TEST_CASE("Factory 產生具正確動態型別的 EnergyDrink") {
...
// 效果延後到「從背包使用」才發生。
TEST_CASE("ConsumableItem 撿取只加入背包，不套用任何效果") {
...
TEST_CASE("IsUsableConsumable 對背包各列道具的可使用分類") {
