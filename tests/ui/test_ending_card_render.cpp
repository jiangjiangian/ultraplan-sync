...
TEST_CASE("IsEndingState 僅對 Ending_* 狀態為 true") {
...
TEST_CASE("DrawEndingCard 會畫出背景、標題、字卡與結算統計") {
    EndingSummary g;
...
TEST_CASE("結局 A 卡會顯示業力與 A 的三項判定條件") {
    EndingSummary g;
...
TEST_CASE("結局 B 冷漠最終選擇只顯示實際生效的條件") {
...
TEST_CASE("結局 B 詛咒傘加負業力會同時顯示兩項生效條件") {
    EndingSummary g;
    g.state = SemesterState::Ending_B;
    g.karma = -7;
    g.tookCursed = true;
...
TEST_CASE("結局 C 買了醜傘顯示購買條件而非預設條件") {
    EndingSummary g;
    g.state = SemesterState::Ending_C;
    g.karma = 50;
    g.boughtUgly = true;
...
TEST_CASE("結局 C 平穩收尾的預設分支顯示「平穩收尾」條件") {
...
TEST_CASE("結局卡恰好畫出一個被選定的「結算」標題作為錨點") {
    EndingSummary g;
    g.state = SemesterState::Ending_A;
    g.karma = 81;
    g.hasTrueUmbrella = true;
...
TEST_CASE("結局 C 畫出醜傘（綠）色塊——即使玩家也體諒了助教") {
...
TEST_CASE("結局 D 與 A／B／C 同樣渲染字卡、緣由與路線") {
    EndingSummary g;
    g.state = SemesterState::Ending_D;
    g.karma = 70;
    g.consoledTA = true;
