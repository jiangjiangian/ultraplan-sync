...
TEST_CASE("CellWidth：全形 CJK 算 2、ASCII 算 1") {
...
TEST_CASE("略寬於對話框的 CJK 單行換成多列，每列寬度都 <=kBoxCells") {
...
TEST_CASE("很長的 CJK 單行換行後沒有任何一列寬於 kBoxCells") {
...
TEST_CASE("ASCII 文字換行只在空白處斷，絕不從字中間斷") {
...
TEST_CASE("Paginate：列依每頁 kBoxRowsPerPage 切成多頁") {
...
TEST_CASE("docs/content 中每句作者撰寫的台詞跨所有頁面都沒有 >28-cell 的列") {
...
TEST_CASE("DrawDialog 一次只算繪一頁的列，且每列都在框內") {
    nccu::DialogState d;
    std::string longLine;
