...
TEST_CASE("TryAdvanceCh3Trade：物物交換鏈每次對話依序推進一環") {
...
TEST_CASE("Ch3 物物交換鏈乾淨地換背包行（香腸 -> 大聲公 -> 無）") {
    EventBus::Instance().Clear();
    Player p = MakePlayer();
...
TEST_CASE("跑圈前找 A 會在劇情中呈現操場跑圈提示") {
...
TEST_CASE("Ch3IndicatorVisible：A->B->C 的「!」一次只揭露一環") {
    Player p = MakePlayer();
...
TEST_CASE("World::UpdateSportsLap：完整跑完操場一圈會設下 Flag_SportsLapDone") {
...
TEST_CASE("ResolveOpenerSubState：Ch3 鏈上 NPC 依各自旗標由 (a)->(b)") {
    Player p = MakePlayer();
...
TEST_CASE("Ch3 清關：領取道具箱的 TrueUmbrella -> 幕間市集且 returnTo 設為 Ch4") {
    EventBus::Instance().Clear();
