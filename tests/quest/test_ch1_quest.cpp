...
// Bug 修正：助教申請書支線——交還申請書設下 Flag_HelpedTA_Ch1（+5 業力、清除申請書旗標），
// 該情分在 Ch3 兌現 +5；先前此旗標從未被任何程式設下，整條助教跨章支線形同虛設。
TEST_CASE("Ch1 交還申請書設下 HelpedTA 情分，並在 Ch3 兌現 +5") {
    Player p = MakePlayer();
    const int k0 = p.GetKarma();

    // 章節／對象／前提不符 → 不觸發。
    nccu::TryReturnTaForm(p, "ta", SemesterState::Chapter2_Midterms);
    CHECK_FALSE(p.HasFlag(nccu::kFlagHelpedTACh1));
    nccu::TryReturnTaForm(p, "victim", SemesterState::Chapter1_AddDrop);
    CHECK_FALSE(p.HasFlag(nccu::kFlagHelpedTACh1));
    nccu::TryReturnTaForm(p, "ta", SemesterState::Chapter1_AddDrop);   // 還沒撿到申請書
    CHECK_FALSE(p.HasFlag(nccu::kFlagHelpedTACh1));
    CHECK(p.GetKarma() == k0);

    // 撿到申請書後交還 → +5、設情分旗標、清除申請書旗標（背包列消失）。
    p.SetFlag(nccu::kFlagFoundForm);
    nccu::TryReturnTaForm(p, "ta", SemesterState::Chapter1_AddDrop);
    CHECK(p.HasFlag(nccu::kFlagHelpedTACh1));
    CHECK(p.GetKarma() == k0 + 5);
    CHECK_FALSE(p.HasFlag(nccu::kFlagFoundForm));        // 申請書已交還

    // 冪等：再次互動不重複加分。
    nccu::TryReturnTaForm(p, "ta", SemesterState::Chapter1_AddDrop);
    CHECK(p.GetKarma() == k0 + 5);

    // Ch3 漣漪：情分在運動會兌現 +5，且只兌現一次（無陷阱傘，故只走 HelpedTA 分支）。
    EventBus::Instance().Clear();
    const int k1 = p.GetKarma();
    nccu::TryApplyCh3Ripple(EventBus::Instance(), p, SemesterState::Chapter3_SportsDay);
    CHECK(p.GetKarma() == k1 + 5);
    CHECK(p.HasFlag(nccu::kFlagCh3RippledTAHelped));
    nccu::TryApplyCh3Ripple(EventBus::Instance(), p, SemesterState::Chapter3_SportsDay);
    CHECK(p.GetKarma() == k1 + 5);                       // 不重複
    EventBus::Instance().Clear();
}
