...
TEST_CASE("TryRescueBookworm：叫醒步驟消耗飲料；換回步驟需要筆記") {
    ...
    CHECK(p.HeldUmbrellaKind() == HeldUmbrella::True);  // 學霸把真傘換回給玩家
...
TEST_CASE("圖書館管理員借出管理員的傘（手持並擋雨，但非真傘）") {
    ...
    nccu::TryLendLibrarianUmbrella(p, "librarian",
                                   SemesterState::Chapter2_Midterms);
    CHECK(p.HeldUmbrellaKind() == HeldUmbrella::None);  // 借傘不佔手持槽（旗標驅動）
    ...
    nccu::TryLendLibrarianUmbrella(p, "librarian",
                                   SemesterState::Chapter2_Midterms);
    CHECK(p.HeldUmbrellaKind() == HeldUmbrella::None);
    CHECK(p.HasFlag(nccu::kFlagLibrarianUmbrella));     // 仍持有（冪等，不重設）
    EventBus::Instance().Clear();
}

// Bug 修正：拿了管理員借傘後通關（學霸換回真傘），兩傘並存——背包同時列出真傘與借傘，
// 借傘不再「蓋過」真傘；歸還借傘 +10 業力且保留真傘。
TEST_CASE("Ch2 借傘與換回的真傘並存，歸還借傘不弄丟真傘") {
    EventBus::Instance().Clear();
    Player p = MakePlayer();
    p.SetFlag(nccu::kFlagMetLibrarian);
    ...
    // 喚醒學霸並向管理員借傘。
    p.AddConsumable("EnergyDrink");
    nccu::TryRescueBookworm(EventBus::Instance(), p, "bookworm",
                            SemesterState::Chapter2_Midterms);             // 喚醒
    nccu::TryLendLibrarianUmbrella(p, "librarian",
                                   SemesterState::Chapter2_Midterms);     // 借傘
    CHECK(p.HasFlag(nccu::kFlagLibrarianUmbrella));
    CHECK(p.HeldUmbrellaKind() == HeldUmbrella::None);   // 借傘只是旗標

    // 撿齊筆記並換回 → 學霸把真傘還回（手持 True），借傘旗標仍在 → 兩傘並存。
    GiveNotes(p);
    nccu::TryRescueBookworm(EventBus::Instance(), p, "bookworm",
                            SemesterState::Chapter2_Midterms);             // 換回
    CHECK(p.HasFlag(nccu::kFlagBookwormRecovered));
    CHECK(p.HeldUmbrellaKind() == HeldUmbrella::True);   // 換回的真傘
    CHECK(p.HasFlag(nccu::kFlagLibrarianUmbrella));      // 借傘仍在（並存）

    // 背包同時列出真傘與借傘兩列——借傘不再蓋過真傘。
    auto rows = nccu::BuildInventoryRows(p);
    auto count = [&rows](const char* id) {
        int n = 0; for (const auto& r : rows) if (r.itemId == id) ++n; return n;
    };
    CHECK(count(nccu::kItemTrueUmbrella) == 1);
    CHECK(count(nccu::kItemLoanerUmbrella) == 1);

    // 在 Ch2→Ch3 插曲段歸還借傘：+10 業力、借傘列消失，但真傘保留（不被順手弄丟）。
    const int k0 = p.GetKarma();
    nccu::TryReturnLibrarianUmbrella(EventBus::Instance(), p,
        nccu::kNpcLibrarianReturn, SemesterState::Interlude_Market,
        SemesterState::Chapter3_SportsDay);
    CHECK(p.GetKarma() == k0 + 10);
    CHECK_FALSE(p.HasFlag(nccu::kFlagLibrarianUmbrella));   // 借傘列消失
    CHECK(p.HeldUmbrellaKind() == HeldUmbrella::True);      // 真傘保留
    CHECK(p.HasUmbrella());                                 // 仍有遮蔽（靠真傘）
    rows = nccu::BuildInventoryRows(p);
    CHECK(count(nccu::kItemTrueUmbrella) == 1);
    CHECK(count(nccu::kItemLoanerUmbrella) == 0);
    EventBus::Instance().Clear();
}
