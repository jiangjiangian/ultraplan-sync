...
TEST_CASE("EventBus delivers events to subscribers") {
    ...
    EventBus::Instance().Publish(Event{EventType::ShowMessage, {0,0}, nccu::gfx::Colors::Black, "hello"});
...
TEST_CASE("EventBus does not deliver to wrong type") {
    ...
    EventBus::Instance().Subscribe(EventType::KarmaChanged,
        [&](const Event&) { hits++; });
    EventBus::Instance().Publish(Event{EventType::ShowMessage, {0,0}, nccu::gfx::Colors::Black, ""});
    CHECK(hits == 0);
}
