...
TEST_CASE("Vendor：ctor 由 greeting 與 stock 項目建立 NPC 對話列") {
...
TEST_CASE("Vendor::TryBuy 成功：扣款並發出 ShowMessage 與 PickupAcquired") {
    Player p({0, 0});
...
TEST_CASE("Vendor::TryBuy 金錢不足：不扣款、ShowMessage 顯示錢不夠、無 pickup 事件") {
    Player p({0, 0});
    // 把錢包清空，使連最便宜的商品都買不起。
...
TEST_CASE("Vendor::TryBuy 索引越界時回傳 false 且不發任何事件") {
    Player p({0, 0});
    const int startMoney = p.GetMoney();
...
TEST_CASE("Vendor::TryBuy 對 null player 為安全的 no-op") {
    VendorCapture cap;
    cap.Attach();
...
TEST_CASE("Factory::Create(ObjectType::Vendor) 回傳 Vendor*") {
